...
def check_uid_availability(metagraph: "bt.metagraph.Metagraph", uid: int, vpermit_tao_limit: int) -> bool:
    ...
    if metagraph.validator_permit[uid]:
        ...
        if stake > vpermit_tao_limit:
            bt.logging.trace(f"UID: {uid} has validator permit and stake {stake} > {vpermit_tao_limit}.")
            return False
...
def get_available_miners(self, k: Optional[int] = None, exclude: list[int] | None = None) -> np.ndarray:
